    ...
    <body bgcolor="#34495E" style="margin: 0; padding: 0;" yahoo="fix">
        ...
        <table align="center" border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse; width: 100%; max-width: 600px;" class="content">
            ...
            <tr>
                ...
                <img src="/assets/images/logo.png/{{Auth::user()->tenant->logo ?? '/assets/images/logo.png'}}" alt="{{Auth::user()->tenant->company_name ?? 'CNX247 ERP Solution'}}" height="75" width="120" style="display:block;">
                </td>
            </tr>
            <tr>
                <td align="center" bgcolor="#ffffff" style="padding: 75px 20px 40px 20px; color: #555555; font-family: Arial, sans-serif; font-size: 20px; line-height: 30px; border-bottom: 1px solid #f6f6f6;">
                    ...
                    <p>Congratulations on your offer from <strong>{{Auth::user()->tenant->company_name }}</strong>! We are delighted to offer you the position of <strong>{{$user->position}}</strong> with an anticipated start date of <strong>{{date('d F, Y', strtotime($user->start_date))}}</strong>.</p>
                    <p>Please find your login credentials to our workflow solution below. When you click on <strong>Activate Account,</strong> you'll be redirected to where you can login. Endeavour to update your profile the moment you login. And don't forget to change your password to something you can remember and secure.</p>
                    <p>
                        <strong>Email address: </strong> <span>{{$user->email}}</span> <br>
                        <strong>Password: </strong> <span>{{$password}}</span> <br>
                    </p>
                    <p>
                        <a href="{{url('/')}}/verify/{{$user->verification_link}}" style="color: #00B2B2; text-align: center; text-decoration: none;">Activate Account</a>
                    </p>
