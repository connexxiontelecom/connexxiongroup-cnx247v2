			...
			if (Hash::check($transactionPassword, Auth::user()->transaction_password)) {
					$details = Post::find($post);
									if($userAction == 'approved'){
										...
										//return response()->json(['rem'=>$remainingProcessors]);
										//identify next supervisor
													...
													//return response()->json(['message'=>$reset[0]]);
													if(count($reset) > 0 ){
							...
							return response()->json(["error"=>"Mis-match transaction password. Try again."],400);
					}
